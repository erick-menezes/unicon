...
export function PostCardWithInteraction({ postData }: PostCardWithInteractionProps) {
    ...
    const [like, setLike] = useState(() => Number(postLikeId?.length > 0));
    ...
    async function getPostDataFromUser() {
        const postLikeId = await PostRepository.getUserLike(postData.id, userAuthSession?.uid!);
        const postSaveId = await PostRepository.getUserSave(postData.id, userAuthSession?.uid!);
    ...
    async function handleRemoveLike() {
        await PostRepository.removeUserLike(postLikeId);
    ...
    async function handleRemoveSave() {
        await PostRepository.removeUserSave(postSaveId);

        setPostSaveId('');
    }

    async function handleAddLike() {
        const likeId = await PostRepository.storeUserLike(
            {
                postId: postData.id,
                userId: userAuthSession?.uid!
            }
        );

        setLike(previous => previous + 1);
        setPostLikeId(likeId);
    }

    async function handleAddSave() {
        const saveId = await PostRepository.storeUserSave(
            {
                postId: postData.id,
                userId: userAuthSession?.uid!
            }
        ...
        setPostSaveId(saveId);
    }

    function getPostTime(postCreatedAt: Date) {
        const daysPassed = moment(moment()).diff(postCreatedAt, 'days');
        
        if (daysPassed <= 0) {
            return `${moment(moment()).diff(postCreatedAt, 'hours')} horas`;
        }

        if (daysPassed > 30) {
            return `${moment(moment()).diff(postCreatedAt, 'month')} meses`;
        }

        if (daysPassed > 360) {
            return `${moment(moment()).diff(postCreatedAt, 'years')} anos`;
        }

        return `${moment(moment()).diff(postCreatedAt, 'days')} dias`;
    }
            ...
            >
                ...
                <Flex flexDirection="column">
                    ...
                    <Text fontWeight="semibold" color="gray.300" display="flex" alignItems="center" gap={1}>
                        ...
                        />
                        há {getPostTime(postData.createdAt)}
                    </Text>
                </Flex>
            </Flex>
